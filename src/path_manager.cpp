...
{
    if( pimpl == nullptr ) {
        debugmsg( "path_manager::pimpl should have NOT been nullptr on saving." );
    }
    std::string name = base64_encode( get_avatar().get_save_id() + "_path_manager" );
...
{
    if( pimpl != nullptr ) {
        debugmsg( "path_manager::pimpl should have been nullptr on loading." );
    }
